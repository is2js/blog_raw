...
#### gradle build Type 변경 -> intellij로 변경(속도 및 TestName표기)
...
### Save Actions / code style / final / saveActions / Auto import /페어 컨벤션 / ~~LF(editorConfig)~~ / ~~checkstyle~~
...
#### (추가) Auto import 설정
- 코드 복붙하거나 할 때 빨간 줄 안없어져서 고생을 많이 했음.
- Settings > Editor > General > **`Auto Import`(바로 검색) 후**
    1. Always -> `Ask`로 변경(복붙후 library 선택)
    2. [x] Add unambiguous import on the fly 체크
    3. [x] Optimize imports on the fly (for current project) 체크
