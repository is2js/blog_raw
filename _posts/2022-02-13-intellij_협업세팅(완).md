...
2. **프로젝트 SQL dialect 설정하기**
    1. `Settings - Languages & Frameworks - SQL Dialects` or `shift 2번(F1) > sql dialect` 선택
...
3. **schema.sql에서 H2를 DataSource로 지정해주기**
    1. 의존성에 h2추가 확인하기
        ```gradle
    ...
    4. **h2를 추가하고, 기본 remote로 지정된 `option을 내장형(In-memory)`로 변경하고, username 등 입력해준다.**
