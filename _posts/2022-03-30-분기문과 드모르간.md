...
### my) boolean이라면, OR(||)에서 앞에것들 통과 못할시 빠르게 처리되도록 -> if로 찢어  `if not return false`시켜버린다.
### my) boolean이라면, if not A -> return false; 이후 `OR(||) 맨 끝에 하나 남은 분기식은 바로 return B`해버려도 된다. ( if not B로직이 필요없는 상황)
